...
gradlePlugin {
    plugins {
        ...
        val conventionPlugins = mapOf(
            "checkstyle-convention" to "plugins.conventions.CheckstyleConventionPlugin",
...
tasks.withType<Test>().configureEach {
    ...
    // Use the type-safe version accessor generated by Gradle for build scripts.
    val checkstyleVersion = libs.versions.checkstyle.get()
    systemProperty("checkstyle.version", checkstyleVersion)

    // Pass the absolute path of the root checkstyle configuration directory.
    // This allows the test harness to copy the entire directory.
    val checkstyleConfigDir = project.gradle.parent!!.rootProject.file("tools/checkstyle").absolutePath
    systemProperty("checkstyle.config.dir", checkstyleConfigDir)
